const generateInsights = (tasks, users, commits, bugs) => {
    ...
    // Insight: Sudden drop in commits
    const recentCommits = commits.filter(c => new Date(c.date) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length;
    const olderCommits = commits.filter(c => new Date(c.date) <= new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) && new Date(c.date) > new Date(Date.now() - 14 * 24 * 60 * 60 * 1000)).length;
    ...
    // Insight: Recommend focus areas
    const openBugsByUser = users.map(user => ({
        name: user.name,
        highPriorityBugs: bugs.filter(b => b.assignedTo === user.id && b.status === 'open' && b.priority === 'high').length
    }));
    const userWithMostBugs = openBugsByUser.reduce((prev, current) => (prev.highPriorityBugs > current.highPriorityBugs) ? prev : current, { highPriorityBugs: 0 });
